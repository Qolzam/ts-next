...
useEffect(() => {
  ...
  if (!init) {
    try {
      var elementId = "audio" + new Date().valueOf().toString();
  ...
  const callBoxElement = () => (
    ...
    <ListItem button onClick={() => {onConnect(userId)}}>
        <ListItemAvatar  >
        <UserAvatar className={classNames('calling', classes.avatar)} 
        fullName={title} size={50} fileName={avatarURL} />
  ...
  return (
    <Snackbar
        ...
        action={ <IconButton aria-label="comments" onClick={() => {onClose(userId)}}>
