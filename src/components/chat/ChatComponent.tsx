...
export class ChatComponent extends Component<IChatProps, IChatState> {
  ...
  sendMessage(message: any) {
    ...
    const receiveLang =  Ramda.path(['translation', receiverUser!.get('userId'), 'input'], currentChatRoom) as string
   ...
   */
  contactList = () => {
    let { connections, classes, receiverUser, users } = this.props
    let parsedDOM: any[] = []
    if (connections && users) {
      connections.forEach((value, key) => {
        const user = users!.get(key)
        const userId = user!.get('userId')
        parsedDOM.push(
          <ListItem onClick={() => this.setCurrentChat(userId)} key={`chat-component-contact-user-${userId}`} button
            ...
            <ListItemText className={classes.listItemText} classes={{ primary: classes.primaryText, secondary: classes.secondaryText }} primary={user.get('fullName', '')} secondary={''} />
          </ListItem>
    ...
    const leftSide = (
      <Grid item sm={6} md={6} lg={6} xl={6}
        ...
        <ListItem classes={{container: classes.currentUserItem}} className={classNames(classes.userItem)}>
            ...
            <ListItemText className={classes.listItemText} classes={{ primary: classes.primaryText, secondary: classes.secondaryText }} primary={currentUser!.fullName} secondary={currentUser!.tagLine} />
        ...
        <List className={classes.listContainer}>
          <ListItem className={classNames(classes.userItem, classes.receiverUserItem)}>
            ...
            <ListItemText
              ...
              primary={receiverUser!.get('fullName')} secondary={receiverUser!.get('tagLine','')} />
            <ListItemSecondaryAction>
              <IconButton
