...
export class CommentComponent extends Component<ICommentComponentProps, ICommentComponentState> {
  ...
  handleUpdateComment = (evt: any) => {
    ...
    this.props.update!(updateComment)
    this.setState({
  ...
  render () {
    ...
    const rightIconMenu = (
      <div className={classes.menuRoot}>
          {this.props.isCommentOwner && (<IconButton
          buttonRef={(node: any) => {
            ...
            <Popover
              ...
              >
                <MenuList role='menu'>
                  ...
                  {(this.props.isCommentOwner || this.props.isPostOwner) ? (<MenuItem className={classes.rightIconMenuItem} onClick={(evt: any) => this.handleDelete(evt, comment.get('id', '0'), comment.get('postId', '0'))}>{t!('comment.deleteButton')}</MenuItem>) : ''}
                </MenuList>
              </Popover>
