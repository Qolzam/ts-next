...
const mapDispatchToProps = (dispatch: any, ownProps: ICommentComponentProps) => {
    return {
        ...
        openEditor: () => dispatch(commentActions.openCommentEditor({ id: ownProps.comment.get('id', '0'), postId: ownProps.comment.get('postId') })),
        closeEditor: () => dispatch(commentActions.closeCommentEditor(ownProps.comment.get('postId'),ownProps.comment.get('id', '0'))),
