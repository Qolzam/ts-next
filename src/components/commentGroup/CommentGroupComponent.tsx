...
export class CommentGroupComponent extends Component<ICommentGroupComponentProps, ICommentGroupComponentState> {
   ...
   */
  handlePostComment = () => {
    const {postId, fullName, avatar, uid} = this.props
    ...
    this.props.send!(Map(newComment))
   ...
   */
   loadCommentsList = () => {
     ...
     const showComments = ((comments && comments.size > 0)  ? (
  ...
  render () {
    ...
    return (
      <div key={postId + '-comments-group'}>
        ...
        {open ? ((commentsRequestStatus === ServerRequestStatusType.Sent) ? commentProgress : this.loadCommentsList()) : ''}
