...
export class EditProfileComponent extends Component<IEditProfileComponentProps, IEditProfileComponentState> {
  ...
  constructor(props: IEditProfileComponentProps) {
    ...
    this.state = {
       ...
       */
      tagLineInput: props.info!.get('tagLine', ''),
      /**
       * User full name input value
       */
      fullNameInput: props.info!.get('fullName', ''),
      /**
       ...
       * User banner address
       */
      banner: props.info!.get('banner',config.settings.defaultProfileCover),
      /**
       * User avatar address
       */
      avatar: props.info!.get('avatar', ''),
      /**
       ...
       * Default birth day
       */
      defaultBirthday: (props.info && props.info.get('birthday')) ? moment.unix(props.info!.get('birthday')).toDate() : '',
      /**
       * Seleted birth day
       */
      selectedBirthday: 0,
       ...
       * Web URL
       */
      webUrl: (props.info && props.info.get('webUrl')) ? props.info.get('webUrl') : '',
      /**
       * User company name
       */
      companyName: (props.info && props.info.get('companyName')) ? props.info.get('companyName') : '',
      /**
       * User twitter id
       */
      twitterId: (props.info && props.info.get('twitterId')) ? props.info.get('twitterId') : '',
      /**
       * User facebook id
       */
      facebookId: (props.info && props.info.get('facebookId')) ? props.info.get('facebookId') : '',
      /**
       * User facebook id
       */
      permission: (props.info && props.info.get('permission')) ? props.info.get('permission'): UserPermissionType.Public,
      /**
       * User facebook id
       */
      accessUserList: (props.info && props.info.get('accessUserList')) ? props.info.get('accessUserList') : []
  ...
  handleUpdate = () => {
    ...
    if (fullNameInput.trim() === '') {
      ...
      update!({
        ...
        creationDate: this.props.info!.get('creationDate'),
        birthday: selectedBirthday > 0 ? selectedBirthday : (info && info.get('birthday' ,0)),
        permission,
        accessUserList,
  ...
  loadCoverList = () => {
    ...
    if (loadCoverList && info && info.get('userId')) {
      loadCoverList(info.get('userId'))
    }
  }
  ...
  loadAvatarList = () => {
    ...
    if (loadAvatarList && info && info.get('userId')) {
      loadAvatarList(info.get('userId'))
    }
  }
            ...
            <div className='profile__edit'>
              ...
              <div className='left'>
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  ...
                  <UserAvatarComponent fullName={(this.props.info ? this.props.info.get('fullName') : '')} fileName={this.state.avatar} size={90} style={this.styles.avatar} />
                </div>
                <div className='info'>
...
const makeMapStateToProps = () => {
  ...
  const mapStateToProps = (state: Map<string, any>, ownProps: IEditProfileComponentProps) => {
    const info = selectCurrentUser(state)
    const avatarImages = selectAvatarImages(state, { userId: info.get('userId')! })
    const coverImages = selectCoverImages(state, { userId: info.get('userId') })
    return {
