...
export class PostWriteComponent extends Component<IPostWriteComponentProps, IPostWriteComponentState> {
  ...
  handlePost = () => {
    ...
    let albumPhotos: Album = new Album()
    if (galleryOpen) {
  ...
  handleDeletePhoto = (fileName: string) => {
    ...
    const textCondition = postText.length === 0 || postText.trim() === '' || (this.props.edit && postText.trim() === this.props.text)
  ...
  isPostChangeValid = (prevState: IPostWriteComponentState) => {
    ...
    return (!StringAPI.isEmpty(image) || !StringAPI.isEmpty(video) || !StringAPI.isEmpty(postText)) || (selectedPhotos && selectedPhotos.length > 0 )
  }
  ...
  render() {
    ...
    const { menuOpen, videoLinkOpen,image, postType,
      ...
      thumbnail, permissionOpen, permission, menuAnchorEl } = this.state
     ...
     * Provide post image
     */
    const loadImage = ((image && image !== '' && postType === PostType.Photo) || (thumbnail && thumbnail !== '' && postType === PostType.Video))
      ? (
        <div>
              ...
              <div style={{ display: 'flex', position: 'relative' }}>
                ...
                <div style={{ display: 'inline-block', width: '100%', marginRight: '8px', transition: 'transform .25s' }}>
                  <li style={{ width: '100%', margin: 0, verticalAlign: 'bottom', position: 'static' }}>
                    <Img fileName={postType === PostType.Photo ? image! : thumbnail!} style={{ width: '100%', height: 'auto' }} />
                  </li>
                </div>
              </div>
          ...
          <DialogActions className={classes.iconButtonsRoot}>
            <div style={{ flexShrink: 0, boxFlex: 0, flexGrow: 0, maxHeight: '48px', width: '100%' }}>
              <div style={{ flexDirection: 'row', display: 'flex' }}>
                ...
                <div className={classes.disableComponent} onClick={this.handleOpenVideoGallery} style={{ outline: 'none', width: '48px', zIndex: 0, overflow: 'hidden', position: 'relative', textAlign: 'center', transition: 'background .3s', border: 0, borderRadius: '50%', display: 'inlineBlock', height: '48px' }}>
