  ...
  getChatRequestsElms = () => {
    ...
    if(chatRequests && users && cancelChatRequest) {
      chatRequests.forEach((value, userId) => {
        ...
        elements.push(<CallSnackComponent 
            key={`chat-request-${userId}`}
            avatarURL={users.getIn([userId, 'avatar'], '')}
            title={users.getIn([userId, 'fullName'])}
            subtitle='Dialing ...' 
            onClose={(userId) => { cancelChatRequest(userId)}}
            userId={userId}
            soundURL={'https://raw.githubusercontent.com/red-gold/red-gold-web/master/website/static/media/notification_sound.mp3'}
            onConnect={() => {}}
            />)
      })
    }
    ...
    if(callingUsers && users && cancelCalling && acceptChat) {
      callingUsers.forEach((value, userId) => {
        ...
        elements.push(<CallSnackComponent 
            ...
            title={users.getIn([userId, 'fullName'])}
            subtitle='Calling ...' 
            onClose={(userId) => { cancelCalling(userId)}}
            userId={userId}
            soundURL={'https://raw.githubusercontent.com/red-gold/red-gold-web/master/website/static/media/notification_sound.mp3'}
            onConnect={(userId) => {acceptChat(userId)}}
            />)
      })
    }
    return elements
