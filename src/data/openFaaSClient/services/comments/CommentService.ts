...
export class CommentService implements ICommentService {
  ...
  public getComments = async (postId: string, page?: number, limit?: number) => {
    try {
      const resultSearch = await this._httpService.get(`comments?postId=${postId}&page=${page! + 1}&limit=${limit}`)
      ...
      if (resultSearch) {
        resultSearch.forEach((comment: any) => {
          parsedData = parsedData.set(comment.objectId!, fromJS({ ...comment, id: comment.objectId, creationDate: comment["created_date"] }))
          commentIds = commentIds.set(comment.objectId!, true)
        })
      }


      return { comments: parsedData, ids: commentIds, newLastCommentId: commentCount > 0 ? resultSearch[0].objectId : '', hasMore: !(commentCount < (limit || 10)) }
