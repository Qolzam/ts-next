...
export class ImageGalleryService implements IImageGalleryService {
  ...
  public getGallery = async (userId: string, folderName: string) => {
    ...
    let newLastImageId = resultExist ? result[0].objectId : ''
  ...
  public saveImages = async (userId: string, folderName: string, photos: Media[]) => {
    ...
    photos.forEach((photo) => {
      ...
      savedPhotos.push(new PhotoModel(
        photo.objectId,
        0,
        0,
        photo.thumbnail,
   ...
   * Fetch album images
   */
  public fetchAlbumImages = async (userId: string, albumId: string, page: number, limit: number = 10, lastPhotoId?: string) => {
   ...
   * Delete file
   */
  public deleteFile= async (userId: string, imageId: string, folderName: string) => {
    try {
