 ...
 */
export const dbUploadVideo = (file: any, fileName: string, thumbnail: Blob) => {
  return { type: ImageGalleryActionType.DB_UPLOAD_VIDEO,payload: {file, fileName, thumbnail} }
}
...
export const dbSaveImage = (imageURL: string) => {
  return (dispatch: any, getState: Function) => {
    ...
    return imageGalleryService.saveFile(uid,newImage, config.data.imageFolderPath)
      .then((mediaId: string) => {
        newImage.objectId = mediaId
 ...
 */
export const dbCreateAlbum = (albumPost: Post ,images: Media[]) => {
  return { 
 ...
 */
export const addVideo = (video: Media) => {
 ...
 */
export const addVideoList = (images: Media[]) => {
