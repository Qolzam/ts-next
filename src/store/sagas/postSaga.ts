...
function* dbSearchPost(query: string, userId: string, lastPostId: string, page: number, limit: number, searchKey: string) {
  ...
  const postResult: { posts: Map<string, any>, ids: Map<string, boolean>, newLastPostId: string, hasMore: boolean } = yield call(postService.searchPosts, query, followingIds.join(), lastPostId, page, limit, searchKey)
  if (!postResult.hasMore) {
